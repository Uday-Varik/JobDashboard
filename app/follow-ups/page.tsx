...
export default function FollowUpsPage() {
  ...
  const renderList = (items: typeof followUps) => {
    if (items.length === 0) {
      return <p className="text-sm text-gray-400 dark:text-gray-500 text-center py-10">None here</p>;
    }
    return (
          ...
          return (
            ...
            >
              <div className="flex items-start gap-3">
                ...
                />
                <div>
                  <p className={cn("text-sm font-medium text-gray-900 dark:text-gray-100", f.completed && "line-through text-gray-400 dark:text-gray-500")}>
                    {app?.company?.name ?? "—"} · {app?.role_title ?? "—"}
                  </p>
                  {f.message && (
                    <p className="text-xs text-gray-500 dark:text-gray-400 dark:text-gray-500 mt-0.5">{f.message}</p>
                  )}
                </div>
              </div>
              <div className="flex items-center gap-1.5 text-xs ml-4 flex-shrink-0">
                ...
                <span className={cn("font-medium", overdue ? "text-red-600 dark:text-red-400" : "text-gray-500 dark:text-gray-400 dark:text-gray-500")}>
  ...
  return (
    <div className="p-8">
      <div className="mb-6">
        <h1 className="text-xl font-semibold text-gray-900 dark:text-gray-100">Follow-ups</h1>
