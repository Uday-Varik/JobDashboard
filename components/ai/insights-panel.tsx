...
export function InsightsPanel({ analytics }: { analytics: AnalyticsData }) {
      ...
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between">
          ...
          <Button variant="ghost" size="sm" onClick={fetchInsights} disabled={loading} className="h-7 text-xs">
            {loading ? <Loader2 className="w-3 h-3 animate-spin" /> : insights ? <RefreshCw className="w-3 h-3" /> : "Analyze"}
          </Button>
        </div>
      </CardHeader>
      <CardContent>
        {insights ? (
          <div className="text-sm text-gray-600 dark:text-gray-400 whitespace-pre-wrap leading-relaxed">{insights}</div>
        ) : (
          <div className="text-sm text-gray-400 dark:text-gray-600 text-center py-4">
            <Sparkles className="w-6 h-6 mx-auto mb-2 text-gray-300 dark:text-gray-700" />
